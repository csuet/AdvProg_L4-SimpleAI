...
char nextCharWhenWordIsNotInDictionary(const set<char>& selectedChars)
{
    ...
    //Write your code here
    for (int i='a';i<='z';i++){
        if (selectedChars.count((char)i)==0) return (char)i;
    }
    return answer;
}
...
char findBestChar(const vector<string>& candidateWords, const set<char>& selectedChars)
{
    ...
    for (int i=0;i<candidateWords.size() ;i++){
        for (int j=0;j<candidateWords[i].size() ;j++){
            if (selectedChars.count(candidateWords[i][j])==0) thongKe[candidateWords[i][j]]++;
        }
    }
...
bool isWholeWord(const string& mask)
{
    ...
    //Write your code here
    for (int i=0;i<mask.size();i++){
        if (mask[i]=='-' or mask[i]=='_') return false;
    }
    return answer;
}
...
bool wordConformToMask(const string& word, const string& mask, char ch)
{
    ...
    //Write your code here
    for (int i=0;i<mask.size();i++){
        if (isalpha(mask[i]) and mask[i]!=word[i]) return false;
    }
    return answer;
}
