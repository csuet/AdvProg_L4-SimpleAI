...
vector<string> filterWordsByLen(int wordLen, const vector<string>& vocabulary)
{
    ...
    for(auto i : vocabulary) {
        if(i.length()==wordLen) {answer.push_back(i);}
    }
    return answer;
}
...
char nextCharWhenWordIsNotInDictionary(const set<char>& selectedChars)
{
    ...
    for(int i = 97; i <= 122; i++) {
        if(selectedChars.find(char(i)) == selectedChars.end()) {answer = char(i); break;}
    }
    return answer;
}
...
char findBestChar(const vector<string>& candidateWords, const set<char>& selectedChars)
{
    ...
    for(int i = 0; i < candidateWords.size(); i++) {
        for(int j = 0; j < candidateWords[i].length(); j++) {
            mp[candidateWords[i][j]]++;
        }
    }
    for(auto i : mp) {
        if(i.second > cnt) {
            if(selectedChars.find(i.first)==selectedChars.end()) {answer = i.first; cnt = i.second;}
        }
    }
    return answer;
}
...
bool isCorrectChar(char ch, const string& mask)
{
    ...
    for(int i = 0; i < mask.length(); i++) {
        if(mask[i]==ch) {answer = true; break;}
    }
    return answer;
}
...
bool wordConformToMask(const string& word, const string& mask, char ch) 
{
    ...
    if(word.length()!=mask.length()) {answer = false;}
        ...
        for(int i = 0; i < word.length(); i++) {
            if(word[i]!='-' && mask[i]!=word[i]) {answer = false;}
        }
    }
    return answer;
}
