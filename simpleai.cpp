...
{
    char answer;
    map <char, int> variable = countOccurrences(candidateWords);
    answer = findMostFrequentChar(variable, selectedChars);
    return answer;
}
...
vector<string> filterWordsByMask(const vector<string>& words, const string& mask, char ch)
{
    ...
    for(string temp : words) {
        if(wordConformToMask(temp, mask, ch)) answer.push_back(temp);
    }
    return answer;
}