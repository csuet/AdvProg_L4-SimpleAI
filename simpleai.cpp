...
char nextCharWhenWordIsNotInDictionary(const set<char>& selectedChars)
{
    char answer;
    for(char c='a';c<='z';c++) { 
        if(selectedChars.find(c)==selectedChars.end()) { // ==map.end() la khong tim thay
            answer=c;
            break;
        }
    }
...
{
    char answer;
    int occurrences_max=0;
    for(char c:selectedChars) {
        int x= occurrences.find(c)->second; //lay gia tri thu 2 cua map la value;
        if(x>occurrences_max) {
            occurrences_max=x;
            answer=c;
        }
    }
