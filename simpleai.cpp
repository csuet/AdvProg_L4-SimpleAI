...
***/
set<char> getRemainingChars(set<char> previousGuesses)
{
...
char selectRandomChar(const set<char> &remainingChars)
{
    int random = rand() % remainingChars.size();
...
{
    char answer;
    set<char> remainingChars = getRemainingChars(selectedChars);
    answer = selectRandomChar(remainingChars);
...
vector<string> filterWordsByMask(const vector<string> &words, const string &mask, char ch)
{
    ...
    {
        if (isCharInWord(ch, word) && wordConformToMask(word, mask, ch))
        {
            answer.push_back(word);
        }
    }
